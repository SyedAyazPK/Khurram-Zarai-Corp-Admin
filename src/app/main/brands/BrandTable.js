...
export default function BrandTable() {
  ...
  const handleChangePage = (event, newPage) => {
    setPage(newPage);
    dispatch(getBrands(`page=${newPage + 1}&limit=${rowsPerPage}`));
  };

  const handleChangeRowsPerPage = (event) => {
    setRowsPerPage(parseInt(event.target.value, 10));
    dispatch(getBrands(`limit=${event.target.value}`));
    setPage(0);
  };
                    ...
                    <TableCell>
                      <div className="flex">
                        ...
                        <FuseSvgIcon
                          ...
                          color="action"
                          onClick={() =>
                            dispatch(deleteBrand(row.id)).then(() =>
                              dispatch(getBrands())
                            )
                          }
                        >
