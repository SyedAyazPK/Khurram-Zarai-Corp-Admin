...
export default function ProductTableDeleted() {
  ...
  const handleChangePage = (event, newPage) => {
    dispatch(getProducts(`page=${newPage + 1}&isDeleted=true`));
    setPage(newPage);
  };

  const handleChangeRowsPerPage = (event) => {
    setRowsPerPage(parseInt(event.target.value, 10));
    dispatch(getProducts(`isDeleted=true&limit=${event.target.value}`));
    setPage(0);
  };
            ...
            <TableCell
              ...
              onClick={() =>
                selected.length > 1 && headCell.label === "Actions"
                  ? dispatch(bulkRecoverProducts(selected)).then(() =>
                ...
                rows.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell>{row.discountedPrice ?? "N/A"}</TableCell>
                      ...
                      <TableCell>
                        <div className="flex">
                          ...
                          <FuseSvgIcon
                            ...
                            onClick={() =>
                              dispatch(
                                bulkRecoverProducts([{ id: row.id }])
                              ).then(() =>
                                dispatch(getProducts(`isDeleted=true`))
                              )
                            }
                          >
                            material-twotone:check
                          </FuseSvgIcon>
                        </div>
                      </TableCell>
                    </TableRow>
