...
export default function CategoryTable() {
  ...
  const handleChangePage = (event, newPage) => {
    setPage(newPage);
    dispatch(getCategories(`page=${newPage + 1}&limit=${rowsPerPage}`));
  };

  const handleChangeRowsPerPage = (event) => {
    setRowsPerPage(parseInt(event.target.value, 10));
    dispatch(getCategories(`limit=${event.target.value}`));
    setPage(0);
  };
              ...
              {categories?.results?.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell>
                      <Avatar src={row.banner?.[0]?.image} />
                    </TableCell>
                    <TableCell align="right">{row?.ParentId?.title}</TableCell>
