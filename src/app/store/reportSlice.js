  ...
  async (report, { dispatch, getState }) => {
    try {
      const response = await axios.get(
        `${REACT_APP_API_URL}/api/v1/orders/yearly?year=2023`
      );
...
export const getOrdersMonthly = createAsyncThunk(
  ...
  async (report, { dispatch, getState }) => {
    try {
      const response = await axios.get(
        `${REACT_APP_API_URL}/api/v1/orders/monthly`
      );
      const data = await response.data;
      return data;
    } catch (error) {
...
export const getSalesYearly = createAsyncThunk(
  ...
  async (report, { dispatch, getState }) => {
    try {
      const response = await axios.get(
        `${REACT_APP_API_URL}/api/v1/sales/yearly?year=2023`
      );
      const data = await response.data;
      return data;
    } catch (error) {
...
export const getSalesMonthly = createAsyncThunk(
  ...
  async (report, { dispatch, getState }) => {
    try {
      const response = await axios.get(
        `${REACT_APP_API_URL}/api/v1/sales/monthly`
      );
      const data = await response.data;
      return data;
    } catch (error) {
...
export const getTopCategories = createAsyncThunk(
  ...
  async (report, { dispatch, getState }) => {
    try {
      const response = await axios.get(
        `${REACT_APP_API_URL}/api/v1/topCategories`
      );
      const data = await response.data;
      return data;
    } catch (error) {
...
const reportSlice = createSlice({
  ...
  extraReducers: {
    [getOrdersYearly.fulfilled]: (state, action) => {
      state.ordersYearly = action.payload.results;
    },
