  ...
  async (customer, { dispatch, getState }) => {
    try {
      const response = await axios.post(
        `${REACT_APP_API_URL}/v1/auth/register`,
        customer
      );
...
export const getCustomers = createAsyncThunk(
  ...
  async (customer, { dispatch, getState }) => {
    try {
      const response = await axios.get(`${REACT_APP_API_URL}/v1/user`);
  ...
  async (customer, { dispatch, getState }) => {
    try {
      const response = await axios.patch(
        `${REACT_APP_API_URL}/api/v1/users/${customer._id}`,
        customer
      );
      const data = await response;
      dispatch(
  ...
  async (searchParams, { dispatch, getState }) => {
    try {
      const response = await axios.get(
        `${REACT_APP_API_URL}/api/v1/users/search`,
        {
          params: {
            q: searchParams,
          },
